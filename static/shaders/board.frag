...
void main(void) {
  ...
  if (block.x >= 0.0 && block.y >= 0.0 && block.x < u_boardsize.x && block.y < u_boardsize.y) {
    vec2 pixel = mod(gl_FragCoord.xy, u_blocksize);
    vec2 tmp = (block + vec2(0.5)) / u_boardsize;
    vec4 color = texture2D(u_board, vec2(tmp.x, 1.0 - tmp.y));
