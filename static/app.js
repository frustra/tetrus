...
(function() {
  var colors, pieces, _ref, _ref1, _ref2, _ref3, _ref4,
  ...
  pieces = [[[true, true, false], [false, true, true]], [[false, true, true], [true, true, false]], [[true, true], [true, true]], [[false, false, true], [true, true, true]], [[true, false, false], [true, true, true]], [[true, true, true, true]], [[false, true, false], [true, true, true]]];

  Tetrus.Piece = (function() {
    function Piece(peer, storage) {
      var piecenum, x, y, _i, _j, _ref2, _ref3;
      ...
      if (!this.storage) {
        ...
        this.width = pieces[piecenum][0].length;
        this.height = pieces[piecenum].length;
        this.storage = new Array(this.width * this.height * 4);
        for (x = _i = 0, _ref2 = this.width; _i < _ref2; x = _i += 1) {
          for (y = _j = 0, _ref3 = this.height; _j < _ref3; y = _j += 1) {
  ...
  Tetrus.Player = (function() {
    function Player(peer) {
      if (!peer) {
        this.nextPiece = new Tetrus.Piece(peer);
      }
  ...
  Tetrus.GamePlayView = (function(_super) {
    ...
    GamePlayView.prototype.viewDidAppear = function() {
      ...
      try {
        ...
        gl.viewportWidth = canvas.width = this.controller.game.board.width * this.blockSize;
        gl.viewportHeight = canvas.height = this.controller.game.board.height * this.blockSize;
      } catch (_error) {
        e = _error;
        console.log(e);
      }
    ...
    GamePlayView.prototype.initBuffers = function() {
      ...
      gl.uniform2f(this.shaders["board"].uBoardSizeUniform, this.controller.game.board.width, this.controller.game.board.height);
    ...
    GamePlayView.prototype.updateBoard = function() {
      ...
      gl.activeTexture(gl.TEXTURE0);
      gl.bindTexture(gl.TEXTURE_2D, this.boardTexture);
      gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, this.controller.game.board.width, this.controller.game.board.height, 0, gl.RGBA, gl.UNSIGNED_BYTE, new Uint8Array(this.controller.game.board.storage));
      gl.activeTexture(gl.TEXTURE1);
      gl.bindTexture(gl.TEXTURE_2D, this.playerOneTexture);
      gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, this.controller.game.player.piece.width, this.controller.game.player.piece.height, 0, gl.RGBA, gl.UNSIGNED_BYTE, new Uint8Array(this.controller.game.player.piece.storage));
      gl.activeTexture(gl.TEXTURE2);
      gl.bindTexture(gl.TEXTURE_2D, this.playerTwoTexture);
      gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, this.controller.game.peer.piece.width, this.controller.game.peer.piece.height, 0, gl.RGBA, gl.UNSIGNED_BYTE, new Uint8Array(this.controller.game.peer.piece.storage));
      gl.useProgram(this.shaders["player1"]);
      gl.uniform2f(this.shaders["player1"].uPiecePositionUniform, this.controller.game.player.piece.position.x, this.controller.game.player.piece.position.y);
      gl.uniform2f(this.shaders["player1"].uPieceSizeUniform, this.controller.game.player.piece.width, this.controller.game.player.piece.height);
      gl.useProgram(this.shaders["player2"]);
      gl.uniform2f(this.shaders["player2"].uPiecePositionUniform, this.controller.game.peer.piece.position.x, this.controller.game.peer.piece.position.y);
      return gl.uniform2f(this.shaders["player2"].uPieceSizeUniform, this.controller.game.peer.piece.width, this.controller.game.peer.piece.height);
    };
